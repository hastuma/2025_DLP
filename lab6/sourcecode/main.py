...
class ConditionalDDPM():
    ...
    def train(self):
        # print(f"train dataset length: {len(self.train_loader)}")
        ...
        for epoch in range(1, self.epoch+1):
            ...
            # acc = self.evaluate(epoch, testing_dataset="test")
            acc = self.evaluate( testing_dataset="test")
            test_acc.append(acc)
            print('Epoch ' + str(epoch) + ' Accuracy: ' + str(acc))
            if acc > bestacc or acc > 0.8 :
                if acc > bestacc :
                    bestacc = acc
    ...
    def evaluate(self, testing_dataset="test"):
        print("testing_dataset : ",testing_dataset)
        test_dataset = iclevrDataset(mode=f"{testing_dataset}")
        ...
        for y in test_dataloader:
            ...
            save_dir = f"./result/{testing_dataset}2/"
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)
            save_image(generated_grid_imgs, "./result/"+testing_dataset+"2/result_"+str(acc*1000)+".jpg")
        return acc
        ...
        for y in test_dataloader:
            ...
            for i, t in tqdm(enumerate(self.noise_scheduler.timesteps)):
                ...
                if(t % 100 == 0):
                    ...
                    save_image(denormalized_x, "./result/2/" + str(t) + ".jpg")
                    img_list.append(denormalized_x)
            grid_img = make_grid(torch.cat(img_list, dim=0), nrow=10)
            save_image(grid_img, "./result/2/progressive_genrate_image.jpg")

            
def main(args):
    ConditionalDDPM = ConditionalDDPM(args)
    print("In testing mode :",args.test)
    if args.test=="True":
        ConditionalDDPM.load_checkpoint()
        # print('DDPM test.json Accuracy : ', ConditionalDDPM.evaluate(testing_dataset="test"))
        print('DDPM new_test.json Accuracy : ', ConditionalDDPM.evaluate(testing_dataset="new_test"))
        ConditionalDDPM.progressive_generate_image()
    else:
        ConditionalDDPM.train()
