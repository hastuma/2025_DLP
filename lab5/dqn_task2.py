...
class DQN(nn.Module):
    ...
    """
    def __init__(self, num_actions,env_name, hidden_channels=512):
        ...
        if env_name == "CartPole-v1":
            ...
            self.network = nn.Sequential(
                ...
                nn.ReLU(),
                # Extra conv layer for richer features, padding=3 to keep 7x7 spatial dims
...
class DQNAgent:
    ...
    def train(self):
        ...
        # Decay function for epsilon-greedy exploration (建議移至 run 方法)
        if self.epsilon > self.epsilon_min:
            self.epsilon *= self.epsilon_decay
        self.train_count += 1
        ...
        actions = torch.tensor(actions, dtype=torch.iextt64).to(self.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save-dir", type=str, default="./task2_best_duplicate_results")
